...
| 生成凭证 | `php artisan admin:authorize {Module} {remark?}`（见 `routes/console.php`，**默认注释**，须去守卫后启用） |
...
2. `AuthorizeRepository` + `ServiceFactory::authorize()`；在 `routes/console.php` **启用** `admin:authorize` 命令（默认注释 + `未开启` 守卫，见 device-authorize.md）
...
7. **未启用 console 命令却执行 artisan** — `admin:authorize` 在 `routes/console.php` 中默认整块注释，须取消注释并去掉 `未开启`/`exit` 守卫
